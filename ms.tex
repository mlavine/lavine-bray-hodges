...
\label{eq:logpost}
  ...
  \end{split}
\end{equation}
Equations~\eqref{eq:reexpress} and~\eqref{eq:logpost} can both be written as a sum of multiples of logs and inverses of linear combinations $\abss$, as in \eqref{eq:sum}, where the summands with $j=s_z+1$ and $j=s_z+2$ are for the terms involving only $\sigesq$ and $\sigssq$, respectively.
\begin{equation}
\label{eq:sum}
...
\item for $j = s_z+2$,
  ...
  \end{align*}
\end{itemize}
Our derivation proceeds from \eqref{eq:sum}.  We use $f$ to denote the target function generically, either~\eqref{eq:reexpress} or~\eqref{eq:logpost}.  When the target function is~\eqref{eq:reexpress}, $c_{s_z+2} = d_{s_z+2} = 0$ and the upper limit of the sum in \eqref{eq:sum} is $s_z+1$.
...
Let $\widetilde{q_4} = (\widetilde{\sigma_e^2},0)$.  For any point $q_4$ having $\sigssq \le \sigma_s^{2*}$ and $\sigesq \ge \widetilde{q_4}$, the partial derivatives (\ref{eq:drlds}, \ref{eq:drlde}) are nonpositive for $j \in \{1, \dots, s_z+1\}$  but nonnegative for $j=s_z+2$.  
\begin{equation*}
...
Combining the requirements for points like $q_1$, $q_2$, $q_3$, and $q_4$, we satisfy desideratum \textbf{D1} for all points in $B^c$ by choosing $\widetilde{\sigma_e^2}$ and $\widetilde{\sigma_s^2}$ sufficiently large. 
