...
%Usual analyses of the posterior density are carried out either by MCMC, in which case one hopes that one's Markov chain happens to sample all regions of high enough posterior density to matter, or by evaluating the posterior on a grid --- in which case one hopes one's grid is fine enough.  With our analysis there is no need to hope; the algorithm is certain to find all regions of high density.
